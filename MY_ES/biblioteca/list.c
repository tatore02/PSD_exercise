...
Item searchList(List l, int a, int *pos){
  *pos=0;
  struct node *p;

  for(p = l->head; p != NULL; p = p->next, (*pos)++)
    if(compareItem2(a,p->value) == 0)
      return p->value;
...
List searchList2(List l, char *a, int *pos){
  *pos=0;
  struct node *p;
  List l1 = newList();

  for(p = l->head; p != NULL; p = p->next, (*pos)++)
    if(compareItem3(a,p->value) == 0)
...
void sortList2(List l){
  ...
  for(p = l->head; p != NULL; p = p->next)
    for(prev = p->next; prev != NULL; prev = prev->next)
      if(compareItem4(p->value,prev->value) > 0)
        swapItem(&(p->value),&(prev->value));
}
